...
const Pedidos = () => {
  ...
  const handleSaveClick = async () => {
    if (editIndex !== null) {
      ...
      const total = cantidad * precio; // Recalcular el total
          ...
          <tbody>
            {filteredPedidos.map((pedido, index) => (
              <tr key={pedido.idPedido}>
                ...
                <td>
                  {editIndex === index
                    ? <input type="text" value={pedido.nombreCliente} onChange={(e) => handleInputChange(e, 'nombreCliente', index)} />
                    : pedido.nombreCliente}
                </td>
                <td>
                  {editIndex === index
                    ? <input type="text" value={pedido.apellidoCliente} onChange={(e) => handleInputChange(e, 'apellidoCliente', index)} />
                ...
                <td>
                  {editIndex === index
                    ? <input type="date" value={new Date(pedido.fechaPedido).toISOString().split('T')[0]} onChange={(e) => handleInputChange(e, 'fechaPedido', index)} />
                    : new Date(pedido.fechaPedido).toLocaleDateString()}
                </td>
                <td>
                  {pedido.total.toFixed(2)} {/* Mostrar el total con 2 decimales */}
                </td>
                <td>
                  {editIndex === index ? (
                    <>
